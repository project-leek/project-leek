<template>
  <div class="add-tag w-full flex flex-col">
    <header class="p-8 flex flex-row">
      <Button back round icon="fas fa-times" class="h-7 w-7" />

      <div class="headlines ml-2 flex flex-col">
        <span>Neuen Tag anlegen</span>
        <span v-if="nfcTag">Tag-ID: #{{ nfcTag.nfcData }}</span>
      </div>
    </header>

    <div class="bg-yellow-400 w-full flex-grow">
      <component
        :is="steps[activeStep]"
        :nfc-tag="nfcTag"
        @update:nfc-tag="updateTag"
        @proceed="activeStep++"
      />
    </div>

    <footer class="flex flex-col items-start items-center">
      <ul
        class="steps flex flex-row justify-center transform -translate-y-3 bg-white rounded-t-md px-2 py-1"
      >
        <li v-for="x in steps.length" :key="x" class="mx-2">
          {{ activeStep === x - 1 ? 'x' : 'o' }}
        </li>
      </ul>

      <div class="actions flex flex-row justify-center mb-4">
        <Button
          v-if="activeStep !== 0"
          round
          :text-size="5"
          icon="fas fa-chevron-left"
          class="mr-4 px-4 py-2 text-center"
          @click="activeStep--"
        />
        <Button
          v-if="activeStep === steps.length - 1"
          round
          class="flex-grow p-2"
          icon="fas fa-download"
          text="Tag erstellen"
          @click="saveTag"
        />
        <Button
          v-else-if="activeStep !== 0"
          class="flex-grow p-2"
          icon="fas fa-chevron-right"
          round
          text="Weiter"
          @click="activeStep++"
        />
        <span v-else class="text-center">Zum Fortfahren bitte NFC Tag an den Reader halten!</span>
      </div>
    </footer>
  </div>
</template>

<script lang="ts">
import { NFCTag } from '@leek/commons';
import { defineComponent, ref } from 'vue';

import AddTagStepDetails from '../components/add-tag/AddTagStepDetails.vue';
import AddTagStepPlaceTagOnReader from '../components/add-tag/AddTagStepPlaceTagOnReader.vue';
import Button from '../components/uiBlocks/Button.vue';
import feathers from '../lib/feathers';

export default defineComponent({
  name: 'AddTag',

  components: {
    Button,
  },

  setup() {
    const nfcTag = ref<NFCTag | null>(null);
    const steps = [AddTagStepPlaceTagOnReader, AddTagStepDetails];
    const activeStep = ref<number>(0);

    const updateTag = (_nfcTag: NFCTag): void => {
      nfcTag.value = _nfcTag;
    };

    const saveTag = async (): Promise<void> => {
      if (!nfcTag.value) {
        return;
      }

      await feathers.service('nfc-tags').create(nfcTag.value);
    };

    return {
      activeStep,
      steps,
      nfcTag,
      updateTag,
      saveTag,
    };
  },
});
</script>

<style lang="css">
.app {
  height: 100vh;
}
</style>
