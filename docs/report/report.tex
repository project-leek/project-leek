...
Dafür werden Tastatureingaben, die die Tag-IDs darstellen, emuliert.
\textit{Evdev}, ein Kernel Modul von Linux, könnte dann zum Abgreifen dieser Eingaben genutzt werden.
Mit \textit{node-evdev}\footnote{https://github.com/sdumetz/node-evdev} kann \textit{evdev} auch mit Node genutzt werden.
Um dieses Modul auch für den Raspberry Pi und mit Typscript nutzbar zu machen, wurde ein \textit{fork}\footnote{https://github.com/anbraten/node-evdev} durchgeführt. Beim \texit{fork} wird das bestehende Repository in ein neues kopiert, damit Änderungen an den Dateien durchgeführt werden können, die jedoch das ursprüngliche Repository nicht verändern. 
\\
...
Ist dieser vorhanden, können dort z. B. User Daten hinterlegt werden.
Dafür wäre ein spezieller NFC-Reader, mit Schreibfunktionalität notwendig.

\subsection{Raspberry Pi}
\subsubsection*{Docker Integration}
Docker Integration sollte für einfachere Auslieferbarkeit angeboten werden (vgl. \ref{abschnitt:docker}).
...
Da sich die öffentliche IP-Adresse eines privaten Internet-Anschlusses in der Regel täglich ändert, wird zum einfachen finden der IP ein DynDns Service benötigt, welcher eine feste Domain in die wechselnde IP Adresse des Routers übersetzt.
...
Alternativ könnte für das Projekt auch \textit{Github.io Pages} genutzt werden.
...
Die E-Mail wäre hierbei ein geeignetes Merkmal, da diese einzigartig ist.
Über gesetzte Scopes in der OAuth Anfrage kann diese vom jeweiligen Provider mitgeliefert werden.
Um die E-Mail als Wiedererkennungsmerkmal zu verwenden, muss vorausgesetzt sein, dass Nutzer immer die gleiche E-Mail bei den unterschiedlichen Providern nutzen.
...
Um ein Wiedererkennungsmerkmal von der jeweilig genutzten Musik zu haben, könnte das dazu gehörige Musik Artwork geladen und gespeichert werden.
Beim primär verwendeten Musikservice Spotify sollte dies kein Problem darstellen, da zu jeder Anfrage von Titeln oder Liedern auch eine Liste von Bildern mitgeliefert wird.\footnote{https://stackoverflow.com/questions/10123804/retrieve-cover-artwork-using-spotify-api}

\subsubsection*{Eigene Bilder nutzen}
Um den Nutzer mehr Personalisierung seiner angelegten Tags zu ermöglichen, könnte dieser eigene Bilder verwenden.
Eigene Bilder hochzuladen sollte generell möglich sein.
Der einfachere Weg hierbei wäre es, dass Nutzer ein externes Bild verlinken, dies würde ohne größerer Aufwand möglich sein.
Sollte der Nutzer eigene Bilder hochladen wollen, würde dies mehr Aufwand beanspruchen, da die Daten zum Backend gesendet un dort gespeichert werden müssten.
In unserem Kontext mit Vue.js und Node.js würde das Plugin \textit{vue-picture-input} helfen.
Mit einem \textit{Axios Post} könnte so das Bild an das Backend gesendet werden. \footnote{https://www.digitalocean.com/community/tutorials/vuejs-uploading-vue-picture-input}.
...
Bei der Ersteinrichtung könnte der Nutzer über die Eingabe der MAC Adresse oder über eine andere festgelegte ID die eigene Box finden und zu seinem Account hinzufügen.
...
Um die Verbindung zu der Box aufbauen zu können, könnte diese auf dem Cloud Backend die entsprechenden Verbindungsdaten hinterlegen.
...
\section{Tools und Vorgehen}
  \subsection{Hilfsmittel}
      ...
      \wrapfiguresafe{r}{0mm}{\includegraphics[width=8cm]{Figma.png}}{Proto\-typisierung mit \textit{figma}}
  ...
  \subsection{Entwicklungszyklus}
    ...
    \\~
    \wrapfiguresafe{r}{0mm}{\includegraphics[width=0.62\textwidth]{Entwicklungszyklus.png}}{Entwicklungszyklus}
...
\section{Technische Umsetzung}
  ...
  \wrapfiguresafe{r}{0mm}{\includegraphics[width=0.45\textwidth]{RaspPi.jpeg}}{Raspberry Pi}
  ...
  \wrapfiguresafe{r}{0mm}{\includegraphics[width=0.25\textwidth]{NfcReaderAndTag.jpeg}}{NFC Reader und NFC Tag}
...
\wrapfiguresafe{r}{0mm}{\includegraphics[width=5.9cm]{HomeScreenSchema.png}}{Schematischer Aufbau Home.vue aus \textit{components}}
\paragraph*{Components}$~$ \\
...
\wrapfiguresafe{r}{0mm}{\includegraphics[width=4cm]{Home.png}}{Home View}
...
\\~\\
\wrapfiguresafe{r}{0mm}{\includegraphics[width=0.45\textwidth]{HomescreenNoTags.jpeg}}{Startseite ohne Tags}
...
\wrapfiguresafe{r}{0mm}{\includegraphics[width=0.45\textwidth]{Settings.jpeg}}{Einstellungen}
...
\wrapfiguresafe{r}{0mm}{\includegraphics[width=0.65\textwidth]{Latex.png}}{Bericht während der Erstellung}
\paragraph*{Bericht} $~$ \\
