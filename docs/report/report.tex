...
Mit dem fünften Meilenstein sollte zwischen dem 14.01.2021 und 28.01.2021 die Entwicklung am Produkt weitgehend abgeschlossen werden, sodass der Fokus mehr auf den Bericht gerichtet werden konnte.

%Ziel
Geplant war hierbei, die letzten Anwendungszenarien final umzusetzen, sodass die letzten beiden Meilensteine höchstens zum Beheben kleinerer Fehler benötigt genutzt werden müsste.
...
An dieser Stelle war es erneut sehr praktisch ein Framework wie \textit{Vue} zu nutzen, da somit die bereits vorhandenen \textit{views} wiederverwendet werden konnten.
Um die Usability zu verbessern, wird dem Nutzer besser signalisiert, ob und welchen Tag dieser angewählt hat.
Dafür wurde in \textit{Home.vue} ein weiteres Attribut \textit{selectedTag} angelegt.
Jedes Tag-Element auf der Startseite hatte einen \textit{OnClick Listener}, welche bei einem Klick auf das jeweilige Element diesen Tag als \textit{selectedTag} setzt und auch eine \textit{Transition} auslöst
\begin{lstlisting}
...
\end{lstlisting}
Um dynamisch die CSS-Klasse der ausgewählten Tags zu ändern, wurde an dieser Stelle bei den \textit{TagEntrys} ein ternärer Operator genutzt.
Je nachdem ob der \textit{TagEntry} der gleiche wie der \textit{selectedTag} ist, werden diesem andere Klasseneigenschaften gegeben.
\begin{lstlisting}
  <TagEntry
...
\end{lstlisting}
Wird nun also ein Tag ausgewählt, bekommen alle anderen die Klasse \lstinline{opacity-25} zugewiesen, was einer Tranzparenz von 25\% entspricht.


Im Sprint wurde auch das Produkt auf Auslieferbarkeit getestet.
Da hierbei das Setup komplett erneut durchgeführt wurde, wurde diese Chance auch gleichzeitig dafür genutzt, ein Setup-Guide\footnote{https://github.com/project-leek/project-leek/blob/docs/Build.md} zu schreiben, sodass andere Nutzer dieses Produkt Zuhause eirichten könnten.

%Probleme
Wie beim vorherigen Meilenstein gestaltete sich auch hier das Zeitmanagement schwierig.
Das Team hätte in diesem Sprint mehr Tickets umsetzen können und mehr Features dem Kunden präsentieren können, als sie diesem zu Beginn versprochen hatten, aber es fehlte die Zeit um die Features genügend zu testen und entsprechend fehlerfrei dem Kunden präsentieren zu Können.
Somit beschloss das Team dem Kunden im Review weniger neue Features vorzustellen als geplant und die zusätzlichen Tickets im nächsten Sprint zu vollenden.
Das Produkt konnte um die Einstellungen der \textit{Leek Box} und der Tags erfolgreich erweitert werden.
Auch das Setup Guide wurde fertiggestellt, sodass ein Nutzer selbstständig eine eigene \textit{Leek Box} einrichten könnte.
Bei dem Design wurden die meisten Unstimmigkeiten beseitigt.
Die restlichen Kleinigkeiten sollten im nächsten Sprint umgesetzt werden.

%Retrospektive
Das Team wurde vom Kunden sehr für das bisher entstandene Produkt sowie deren gesamte Produktivität gelobt.
Besonders weil das Team soviel Gefallen an dem Projekt gefunden hat und entsprechend viel Energie aufgewendet hat, war dieser Lob sehr wertvoll und motivierend.
