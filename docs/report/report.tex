...
%Abbbildung mit rudimentärem Beispiel einer vue component
%Beschreibung: Beispiel einer simplen \textit{vue component}-Datei.
%In dieser ist der entsprechenden HTML-, TypeScript- und CSS-Code der Komponente hinterlegt.
%Alternativ kann auch JavaScript genutzt werden.
%In beiden Fällen handelt es sich bei diesem Teil um Framework-spezifischen Programm-Code,
% wie z. B. die Registrierung der Komponente.
...
Dies wird in Abbildung \colorbox{red}{TODO} dargestellt.
...
Der mitgelieferte Wert des Events wird mot \textit{\$event} ausgelesen und zur Veranschaulichung in der Konsole ausgegeben.
\\~\\
Alles in allem ermöglichten \textit{components} dem Team schlanken und übersichtlichen Frontend-Code zu produzieren.
\begin{lstlisting}
[caption={beispielhafte Listen-Komponente (Liste.vue)}, captionpos=b]
  <template>
    <eintrag :name="'eins'" @eintrag-geklickt="geklickt($event)" />
    <eintrag :name="'zwei'" @eintrag-geklickt="geklickt($event)" />
    <eintrag :name="'drei'" @eintrag-geklickt="geklickt($event)" />
  </template>
...
\begin{lstlisting}
[caption={beispielhafte Eintrag-Komponente (Eintrag.vue)}, captionpos=b]
  <template>
...
Wie das Beispiel in Abbildung \colorbox{red}{TODO} demonstriert, können so Datenänderungen sehr einfach angezeigt werden.
Hier wird bei Instanziierung der Komponente (\textit{setup})
%Beispiel
\\
