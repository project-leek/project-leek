...
Um möglichst viel parraleles Arbeiten und Eigentständigkeit der Tickets zu ermöglichen, wurde jede Ansicht in seine einzelnen Komponenten aufgeteilt. Diese Komponenten sollten für sich alleine stehend funktionieren und so anpassbar sein, dass z.B nur ein Button Komponent entwickelt werden musste, dass auf allen Ansichten verwendet werden konnte.
Es wurde daher erst die grundliegenden Komponenten wie, Buttons, Textfields, Dropdowns entwickelt um aus diesen Komponenten die Views zu erstellen.
...
\paragraph*{Komponents am Beispiel des Buttons}$~$ \\
% man könnte eventuell noch über die Probleme und die Entwicklung des Button Komponents sprechen? Wüsste aber nicht wie ich das vernüftig in Worte fasse ohne zu schreiben, dass wir uns doof angestellt haben und zum Schluss das Komponent viel küzer fassen konnten.
Das Button Komponent war eines der vielseitigsten Kompontenten die im Projekt genutzt wurde. Es gab folgende Variatonen: \\
\begin{itemize}
  \item nur Text
...
\end{itemize}
Wetierhin musste die Größe des Buttons anpassbar, aber trotzdem möglichst einheitlich über die einzelnen Views hinweg.
Mit diesen Anforderung hat sich das Team dafür entschieden eine einzige Vue Klasse zu schreiben, die alle Anforderungen erfüllt.
Das Vue Component besteht aus einem leeren Rahmen der lediglich das Design vorgibt und Icon und Text annimmt.
...
\end{wrapfigure}
Interessant an dieser Stelle sind die dynamischen Abfrage \glqq v-if\grqq mit denen geprüft wurde ob ein Text, ein Icon oder beides vorhanden ist. Entsprechend dieses Ergebnis wurden unterschiedliche CSS-Klassen dem Objekt zugeschrieben und daher der Button in seiner Form verändert. Ein Button der nur ein Icon enthielt, sollte zum Beispiel vollständig Rund und das Icon zentriert sein.
Bei der Benutzung des Komponents musste also nur der Inhalt übregeben werden und der Rest geschah automatisch.
\\

Um die Größe des Buttons zu steuern wurde sich ebenfalls dafür entschieden ein prop zu nutzen. So konnte an das Komponent das Attribut \glqq size \grqq übergeben werden mit den Werten: \glqq xs\grqq , \glqq md\grqq und \glqq xl\grqq. Mit diesem Attribut konnte ein Button für die Welcome-Page deutlich größer sein als ein Speichern Button in der App selbst, und trotzdem wurde sichergestellt dass es nur eine Art von \textit{größerem} Buttom existiert.

% Beispiel eines einzelnen Components
% wie haben wir es ermöglicht das der Button auf allen Views verwenbar ist: style props und text reingeben und slots
\paragraph*{Umsetzung des Homescreens}$~$ \\
\begin{wrapfigure}{r}{0cm}
  \includegraphics[width=5.9cm]{HomeScreenSchema.png}
  \caption{Schematischer Aufbau Home.vue}
  \label{fig:HomeVueSchema}
\end{wrapfigure}
...
\\~\\
\begin{wrapfigure}{r}{0cm}
  \includegraphics[width=6cm]{AddSteps.png}
  \caption{AddTag components}
  \label{fig:AddStepStrack}
\end{wrapfigure}
...
\\~\\
Das Erste \textit{component} ist \textit{TagStepPlaceTagOnReader}. Dieser beinhaltet einen Text und ein \textit{GIF}, welches den Benutzer zum Scannen des
NFC-Tags auffordert. Im \textit{TypeScript}-Code wird im \textit{Lifecycle Hook onMounted} an das \textit{patch}-Event des NFC-Readers der Aufruf der Funktion
\textit{attachedTagListener} gebunden. Diese prüft, ob ein NFC-Tag mit dieser ID bereits in der Datenbank vorhanden ist und leitet den Benutzer in diesem Fall
an die \textit{View} zum Bearbeiten des Tags weiter. Ist der NFC-Tag noch nicht angelegt wird die ID an die \textit{ID-Property} des \textit{newTag} gebunden
und das nächste \textit{component} geladen.
\\
Dies ist das \textit{TagStepInfo component}. Es beinhaltet ein \textit{Textbox component} für den Namen des NFC-Readers und ein \textit{DropDown component},
