...
wurde. Sie behinhaltete Attribute wie \glqq Name \grqq{} oder \glqq Id \grqq. Auf Basis dieser Klasse wurde anschließend ein Feathers-Service in Form einerseits
Klasse erstellt. Diese implementierte das von \textit{Feathers} bereitgestellte generische Interface \textit{Service<T>}. Als Typparameter war hier
die zuvor erstellte Modell-Klasse anzugeben. Um Objekte der Klasse \textit{NicksPet} zu verwlaten wurde folgender Service angelegt:
\begin{lstlisting}
...
verwaltet werden sollen (in diesem Fall mit der durch \textit{Feathers} bereitgestellten Schnitstelle für die Datenbank \textit{NeDB}).
...
und die Arbeit im Backend abgeschlossen. Der nächste Schritt behinhaltete die Implemntierung der Steuerelemente zum Verwalten der Daten im Frontend.
Aufgrund der erhöten Komplexität der Einarbeitung in mehrere teilweise komplett unbekannte Frameworks zur selben Zeit, bestanden bei dieser Aufgabe
einige Startschwierigkeiten, welche allerdings durch große Hilfsbereitschaft der erfahreren Teammitglieder überwunden werden konnte.
\\~\\
Im Frontend wurde pro Haustier je ein \textit{compontet} zur Anzeige eines Haustiers angelegt,
welches zusmmen mit den Steuerelementen zum Anlegen, Löschen und Bearbeiten in einer \textit{View} Platz fand.
Aufgrund der je nach Entwickler sehr unterschiedlichen Implmentierung des Frontends wird hier auf eine ausführliche Beschreibung der \textit{components} verzichtet.
%\\~\\
\paragraph*{NFC-Tags auslesen} $~$ \\
\colorbox{red}{Hier bei Bedarf noch weiter ausführen}
Neben der Umsetzung des Hello-Worlds sollte sich mit dem Raspberry Pi und dem NFC-Reader auseinandergesetzt werden und die
Möglichkeit NFC-Tags auszulesen implementiert werden. Diese Anforderung geriet mangels ausreichender Notizen beim ersten Review vorerst
in Vergessenheit, sodass das Team sich einige Tage vor Sprintende nicht sicher war, ob diese Anforderung bis zum Review umgesetzt werden konnte.
...
analog zu dem in \ref{HelloWorld} beschriebenen Vorgehen angelegt.
...
Eingabegeräte vom Linux-Kernel, wie in unserem Fall den NFC-Reader, bereitstellt. Dies ist möglich, da der NFC-Reader sich wie eine Tastatur verhält. Der NFC-Reader
simmuliert dabei eine Tastatureingabe mit dem Text der NFC-Tag-ID. Zur Nutzung wurden zwei Klassen erstellt. Zum einen die NFC-Reader Klasse
(im NFC-Reader Package), welche jeden simmulierten Tastendruck des NFC-Readers abfängt bis ein Zeileumbruch (0A HEX) empfangen
wird und anschließend ein Event auslöst, welches die eingescante ID beinhaltet. Die Hauptklasse nimmt dieses Event entgegen (Event-Listener) und setzt mittels des
NFC-Reader Services den aktuell anliegenen Tags (\textit{currentTag}) auf die übergebene ID. Durch die Nutzung von \textit{Web-Sockets} und \textit{Vue.js}
...
folgenden Milestone eingeplant und vorerst die Anzeige der NFC-ID als JSON-String umgesetzt.
\\
\colorbox{red}{Hier bei Bedarf noch weiter ausführen (aber wäre sehr ins Detail gegeangen)}
Für den Microservice wurde anschließend ein Docker-Image kreiert,
welches eine einfache und gleichzeitig optionale Auslieferung zur restlichen Installation ermöglicht. Dies bringt den Vorteil, dass auch auf einem anderen
System (wie z. B. einem esp32) integrierte NFC-Reader, statt des aktuellen genutzt werden können.
