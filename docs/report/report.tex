...
Mit diesem Sprint sollte das User Interface auf die letzten Unstimmigkeiten im User Interface beseitigt, alle Fehlerbehebung abgeschlossen und somit die Entwicklung beendet werden, sodass der Fokus komplett auf den Bericht gelegt werden kann.
...
\\~\\
%Entwicklung
Zu Beginn wurden die letzten Softwaretickets erledigt. Vor allem im User Interface wurden noch Kleinigkeiten hinzugefügt oder behoben.
Auf allen Seiten wurde die Höhe des \textit{Header} und \textit{Footer} angepasst, sodass diese immer die gleiche Größe besitzen.
\\
...
\begin{itemize}
  ...
  \item Text mit Ion rechts vom Text
...
\begin{wrapfigure}{r}{0cm}
  \includegraphics[width=5.9cm]{Screenshot_Button.png}
  \caption{Codesnippets vom Button}
  \label{fig:HomeVueSchema}
\end{wrapfigure}
Interessant sind die dynamischen Abfragen \glqq v-if\grqq mit denen geprüft wird, ob ein Text und oder ein Icon vorhanden ist.
Entsprechend dieses Ergebnises wurden dem Objekt unterschiedliche CSS-Klassen zugewiesen wodurch der Button seine Form verändert.
...
Aus der Datenbank wurden die manuell generierten Einträge gelöscht.
Auf der Startseite wurde nun dem User auch ein Hinweis angezeigt, falls dieser noch keine NFC-Tags angelegt hat.
\paragraph*{Flow-Charts} $~$ \\
\begin{wrapfigure}{r}{0cm}
  \includegraphics[width=0.45\textwidth]{HomescreenNoTags.jpeg}
  \caption{Startseite ohne Tags}
\end{wrapfigure}
...
\paragraph*{Flow-Charts} $~$ \\
\begin{wrapfigure}{r}{0cm}
  \includegraphics[width=1\textwidth]{HomescreenDesktop.jpeg}
  \caption{Desktopansicht}
\end{wrapfigure}
Das Frontend wurde vom Raspberry zu Github.io Pages umgelagert, um somit eine einfachere allgemeine Webseite den Endkunden zu stellen.
So hat der Endnutzer immer eine einzige URL, die er sich merken muss, und muss sich auch nicht um weitere Einstellungen des Heimnetzwerkes kümmern, um auf seine \textit{Leek Box} von außerhalb zugreifen zu können.
Die Verbindung zwischen Frontend und Backend wurde auch anstelle einer Ip Adresse ersetzt durch ngrok.
Ngrok veröffentlichte hierbei automatisch eine öffentliche verschlüsselte IP Adresse, um diese im Frontend eingeben zu können.
Hierbei war noch die Problematik aufgetroffen, dass diese IP Adresse nicht überprüft wurde.
Hat nun der Nutzer eine falsche IP Adresse eingegeben, versuchte das Frontend sich mit einer nicht existenden IP Adresse zu verbinden und der Fehler 404 wurde angezeigt.
Vor allem problematisch war es hier auch, dass bei Neustart des Raspberrys auch ngrok neugestartet wurde und somit sich die IP Adresse änderte.
Bisher musste der Nutzer auch seinen lokalen Cache löschen, da sonst der Browser weiterhin versuchte, sich mit der alten IP Adresse zu verbinden.
Dieses Problem wurde in den nächsten Meilenstein geschoben.

%Bericht
Anfänglich wurden die Kapitel und Abschnitte für einen insgesamt runderen roten Faden im Bericht überarbeitet.
Aus diesen wurden entsprechend die Tickets im Ticket Pool erstellt, die sich die einzelnen Teammitglieder zuweisen können.
Dies hatte den Vorteil, wie bei der Softwareentwicklung auch bei der Dokumentationsarbeit eine Übersicht zu haben, welche Teile des Berichtes bereits bearbeitet werden, welche noch offen sind, wieviel noch zu tun ist.
...
Das hatte zur Folge, dass zum Ende des Sprints für den Prüfenden nicht die erste Version des Berichtes vorlag und das Team somit verspätet erstes Feedback bekommen konnte.
Nur etwas mehr als die Hälfte das Berichtes konnte umgesetzt werden.

%Retrospektive
Dass der Bericht mit latex geschrieben wurde hatte den Vorteil, dass hier auch Git sinnvoll genutzt werden konnte und die Änderungen der jeweiligen Mitglieder auch schnell ersichtlich waren.
So konnte auch weiterhin der Workflow genutzt werden, dass mit Pull Requests und mindestens zwei Reviewern gearbeitet werden muss.
