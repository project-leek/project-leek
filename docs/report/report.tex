...
Hierfür könnte \textit{evdev} genutzt werden, ein Kernel Modul von Linux. Mit \textit{node-evdev}\footnote{https://github.com/sdumetz/node-evdev} kann \textit{evdev} auch mit Node genutzt werden. Mit dem Fork\footnote{https://github.com/anbraten/node-evdev} von Anbraten wird auch Raspberry Pi und Typescript Unterstützung gestellt.
Hierbei werden Keyboard Eingaben emuliert, die die Tag-IDs senden.
...
Um diese Web Anwendung von außerhalb des eigenen Netzwerkes erreichen zu können, muss innerhalb des Routers der Port 80 ge-forwarded werden.
...
Um von der Cloud Anwendung auf die Daten vom NFC Reader der verschiedenen Boxen zuzugreifen, wäre eine API sinnvoll.
Der Chip der Box könnte beim auslesen von neuen NFC-Tag Daten einen API Call an die Cloud Anwendung schicken, welche das jeweilige hinterlegte Event zu triggern.
...
Um die E-Mail als Wiedererkennungsmerkmal zu verwenden, muss vorausgesetzt sein, dass Nutzer immer die gleiche E-Mail bei den unterschiedlichen Providern nutzen. Dies ist aber nicht immer die Regel.
Daher könnte dem (bereits eingeloggten) Nutzer die Möglichkeit gegeben werden, weitere Accounts zu dem bestehenden hinzuzufügen.
Dabei könnte dann der neue Account zu dem bereits bestehendem Account hinzugefügt und entsprechend in der Datenbank hinterlegt werden.
...
Der Box wäre dann die jeweilige Box bekannt und könnte über zum Beispiel den vom Nutzer gewählten Namen wiedergefunden und ausgewählt werden.

\subsubsection{Boxdaten über Cloud Anwendung ändern}
Um die auf der Box gespeicherten Daten aus der Cloud Anwendung heraus zu ändern, könnte eine API genutzt werden.
...
Hierfür wäre allerdings ein entsprechender Lautsprecher am Raspberry erforderlich. Ein vergleichbares System zu Spotify Connect existiert derzeit in dieser Form noch nicht für Youtube.
