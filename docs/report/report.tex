...
\subsubsection{Ziel}
Wie auch beim vorherigen Meilenstein war wieder hier der Fokus auf den Kundenwunsch, dass die ersten Funktionalitäten umgesetzt werden.
Dass umfasste das Anlegen von eigenen Tags, also dem Scannen von Tags, vergeben eines eigenen Namens, auswählen von Musik und einem eigenen Bild, sowie das bearbeiten bereits angelegter Tags.
...
Die Planung und Einschätzung der umsetzbaren Tickets wurde schlecht eingeschätzt.
...
Gleichzeitig wurden hierbei auch für eine leichtere Weiterarbeit jegliche Komponenten als möglichst allgemeine Vue Komponenten angelegt, um ein einheitliches Design zu ermöglichen.
Für die Entwickler wurden auch die Pull Request Tests um Typescript Support erweitert.
\subsubsection{Retrospektive}
...
Pull Requests und Tickets wurden besser beschrieben, sodass Probleme besser verständlicher waren.
Vor allem durch das interne Team Review wurde die Zusammenarbeit weiter gebessert.
...
Das Zeitmanagement war an dieser Stelle nicht gut, das Team hatte sich zu viel vorgenommen und am Ende zu wenig Zeit.
...
Der Meilenstein fünf vom 14.01.2021 bis zum 28.01.2021 sollte dafür dienen, die Entwicklung am Produkt weitgehend zu beenden, sodass sich das Team darauf auf den Bericht fokussieren konnte.
\subsubsection{Ziel}
Geplant war, die letzten Anwendungszenarien final umzusetzen, sodass die letzten zwei Meilensteine höchstens zum Beheben kleinerer Fehler benötigt werden würden.
Somit war das Ziel die Umsetzung der Einstellungen für die \textit{Leek Box} selbst als auch der einzelnen Tags.
Die User Interface Elemente sollte, sofern noch nicht geschehen, ein einheitliches Design erhalten.
Auch sollte die Dokumentation zur Bedienung der \textit{Leek Box} fertig gestellt sein und die Arbeit für den Bericht beginnen.
\subsubsection{Probleme}
Wie beim vorherigen Meilenstein hat auch hier das Zeitmanagement nicht komplett funktioniert.
Gegen Ende fehlte es an Zeit, um neue umgesetzte Tickets auf Fehler zu testen und entsprechend fehlerfrei dem Kunden zu präsentieren.
\subsubsection{Lösungen}
...
\subsubsection{Product Increment}
Die \textit{Leek Box} Einstellungen sind implementiert worden, über die der Nutzer den Wiedergabelautsprecher auswählen, die jeweilige Leek Box verwalten und sich auch ausloggen kann.
Auch wurde ein Setup Guide angefertigt, sodass ein Nutzer selbstständig eine eigene \textit{Leek Box} einrichten könnte.
Bei dem Design wurden alle Unstimmigkeiten noch beseitigt.
\subsubsection{Retrospektive}
Insgesamt hat das Team sich an dieser Stelle vor allem über das sehr positive Feedback des Kunden gefreut.
Auch war die Produktivität des Teams sehr gut.
