...
Durch die starke Typisierung entstehen außerdem weniger Laufzeitfehler, da die entwickelnde Person bereits zur Compilezeit auf Typfehler aufmerksam gemacht wird.\cite{Typescript_Typisierung}
...
Neben der Programmiersprache Typescript setzte das Team verschiedene Frameworks ein, um bei der Entwicklung der Benutzeroberfläche mit weniger Aufwand zu entwickeln.
...
Dieses wurde gewählt, da es sehr gut dokumentiert ist und eine flache Lernkurve besitzt, wodurch der Einstieg für Teammitglieder mit weniger Erfahrung in der Web-Entwicklung leichter war.
...
Zur Vereinfachung der Gestaltung der Benutzeroberfläche entschied sich das Team für das Utility-First-Frameworks \textit{TailwindCSS}\footnote{\raggedright\url{https://tailwindcss.com}}.
...
über die das Design selbst definiert werden kann.So kann das Aussehen von Elementen innerhalb des HTML-Codes festgelegt werden.
...
Das Framework \textit{feathers.js}\footnote{\raggedright\url{https://feathersjs.com/}} ermöglicht CRUD-Zugriffe auf verschiedene Services, welche zum Beispiel Datenbanken oder externe APIs sein können.
...
Es muss lediglich ein Service erstellt werden, der das generische Interface Service mit Typeparameter der zu verwaltenden Klasse implementiert.
\begin{figure}[h]
  \includegraphics[width=\linewidth]{MyFeathersService}
  \caption{Implementierung eines Services}
  \label{fig:MyService}
  \protect\cite{}
\end{figure}
...
Dadurch kann der Server alle verbundenen Clients bei einer CRUD-Zugriff informieren, sodass alle ohne erneutes Anfrage der Daten den Aktuellsten Zustand übermittelt bekommen.
Wird also beispielsweise ein NFC-Tag von Person A geändert, sieht Person B diese Änderung ohne Aktualisierung der Seite.
Eine weiteres Feature, welches vom Projektteam genutzt wird sind die integrierten OAuth-Provider-Abstraktionen, die ein einfaches Authentifizieren mit Diensten wie in unserem Fall \textit{Spotify} ermöglichen.

\subsubsection{Reverse-Tunnel (ngrok)}
Um verschlüsselt vom Frontend (öffentlich unter \raggedright\url{https://project-leek.github.io} erreichbar) auf das jeweilige Backend einer \textit{leek-box} zuzugreifen, wird der Reverse-Tunnel Dienst \textit{ngrok}\footnote{\raggedright\url{https://ngrok.com/}} verwendet.
...
sodass eine weitere Installation seitens der Benutzer:innen nicht erforderlich sind.
