...
verwaltet werden können, musste die Steuerung des NFC-Readers und ein System zur Speicherung der Daten und zum Abspielen der Musik entwickelt werden.
Die dafür benötigten Anwendungen wurden als Microservices konzipiert. Zur Verringerung des Aufwands der Konfiguration und Wartung wurden die einzelnen Microservices in einem Mono-Repository auf GitHub zusammengefasst.
Jeder Service wurde in einem Unterdner des Packages Ordner angelegt (vgl. \ref{fig:Orderstruktur}). Der Ordner \textit{app} behinhaltet die Benutzeroberfläche,
\textit{backed} die Datenbankschnittstelle und Logik zur Kommunikation mit dem Musik-Streaming-Anbieter und \textit{nfc-reader} die Applikation zum Steuern und auslesen des am Raspberry Pi angeschlossenen NFC-Reader.
Der Ordner commons beinhaltet die von Backend, Frontend und NFC-Reader gemeinsam genutzten Ressourcen (wie z. B. Modell-Klassen).
Damit waren die einzelnen Services für jeden Entwickler jederzeit einfach erreichbar, ohne die Notwendigkeit das Repository zu wechseln.
