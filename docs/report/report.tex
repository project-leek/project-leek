...
Möglichkeit NFC-Tags auszulesen implementiert werden. Zur Umsetzung wurde im Backend die Klassen und der Service zur Verwaltung eines NFC-Readers und NFC-Tags
...
angelegt. Er nutzt die Bibliothek \textit{Evdev}\footnote{https://github.com/PixnBits/node-evdev}, die Events für die angeschlossenen
Geräte, wie in unserem Fall den NFC-Reader bereitstellt. Zur Nutzung wurden zwei Klassen erstellt. Zum einen die NFC-Reader Klasse (im NFC-Reader Package), welche
von der Klasse \textit{events} erbt. Dies ermöglicht es auf Instanzen dieser Klassen einen EventHandler zu registrieren. Die Klasse
ethält als Attribut eine Instanz von \textit{Evdev}, welches das \textit{on} Event des physischen NFC-Readers an das \textit{on} Event
der Klasse bindet. Letzteres kann mittels eines EventListeners das angelegte NFC-Tag als das \textit{currentTag} setzten.
\colorbox{red}{Irgendwie so......@Anton?}.
Für den Microservice wurde dann ein Docker-Image kreiert, welches die einfache Auslieferung zur Installation ermöglicht.
Diese Anforderung geriet mangels ausreichender Notizen beim ersten Review vorerst
