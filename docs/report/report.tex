...
Mit diesem Sprint sollte das User Interface auf einheitliches Design, Fehlerbehebung abgeschlossen und somit die Entwicklung beendet werden, sodass der Fokus komplett auf den Bericht gelegt werden kann.
So würde der folgende und letzte Meilenstein für letzte Korrekturen am Bericht und das Vorbereiten der Projektpräsentation genutzt werden.

%Entwicklung
\colorbox{red}{TODO:} Database ohne seeds, anwndung auf raspi
Zu Beginn sollten die letzten Softwaretickets erledigt werden.
Vor allem im User Interface wurden noch Kleinigkeiten hinzugefügt oder behoben.
Auf allen Seiten wurde die Höhe des \textit{Header} und \textit{Footer} angepasst, sodas diese immer gleich ist.
Der \textit{Button Component} wurde überarbeitet, dass auch dieser universell für alle Anwendungszwecke benutzt werden kann
Auf der Startseite wird dem User eine entsprechende Nachricht angezeigt, sollte dieser noch keine NFC-Tags angelegt haben.
\colorbox{red}{siehe Bild, noch einfügen zum zeigen}
Auch wurde die Desktop Ansicht optimiert:\\
Da unsere Anwendung für die mobile Ansicht optimiert ist, wird in der Desktop Ansicht die Anwendung als kleineres Fenster im Handy Format angezeigt.
Diese streckte sich auf höher aufgelösten Bilschirmen zu sehr in die Höhe im Vergleich zur Breite.
...
Die Verbindung zwischen Frontend und Backend wurde auch anstelle einer Ip Adresse ersetzt durch ngrok. \colorbox{red}{was genau macht ngrok?}
Ngrok veröffentlicht hierbei automatisch eine öffentliche verschlüsselte IP Adresse, um diese im Frontend eingeben zu können.
Hierbei ist noch die Problematik aufgetroffen, dass diese IP Adresse nicht überprüft wird und bisher nur Fehler 404 angezeigt wird, falls dieser falsch ist.
Vor allem problematisch ist es hier noch, dass bei Neustart des Raspberrys auch ngrok neugestartet wird und somit sich die IP Adresse ändert.
Bisher muss nun der Nutzer seinen lokalen Cache löschen, da sonst der Browser weiterhin versucht, sich mit der alten IP Adresse zu verbinden.
Das wird im nächsten Meilenstein gelöst.
...
%Product Increment
\colorbox{red}{Leekbox auswählbar, letzte bugfixes}
