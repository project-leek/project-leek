...
Dabei entdeckten die Entwickler die Biblothek \textit{Spotify Web API node} (vgl. \ref*{SpotifyWebApiNode}), welche den Zugriff auf die \textit{Spotify-API},
die die Nutzung abstrahierte und so den Zugriff erleichterte. Die Bibliothek wurde in Kombination mit \textit{Feathers-Hooks}
\footnote{https://docs.feathersjs.com/api/hooks.html}. Hooks sind Funktionen, die z. B. vor oder nach einem Feathers Service Aufruf ausgeführt werden können.
In diesem Fall beinhaltet der angelegte \textit{playSpotify} Hook einen Aufruf der Methode \lstinline{play} der \textit{Spotify Web API node} die den Song
hinter der url des mitgegebenen NFC-Tags abspielt. Der Aufruf geschiet automatisch nachdem die Methode \textit{patch} des NFC-Reader Services aufgerufen wurde.
\colorbox{red}{Hier noch was dazu oder was falsch @Anton?}

\paragraph*{NFC-Tags scannen emulieren} $~$ \\
Da während des Projekts nicht alle Entwicker kontunierlichen Zugriff auf einen NFC-Reader hatten wurde im Zuge dieses Milestones außerdem die Möglichkeit
geschaffen, das Scannen eines NFC-Tags zu emulieren. Hierfür wurde ein \textit{Batch-Script} erstellt, welches einen \textit{HTTP PATCH request} an
das Backend schickt. Dadurch wird der \textit{attachedTag} des NFC-Readers auf Basis eines übergebenen Parameters gesetzt. So kann ein NFC-Tag mit der
ID 12345 emuliert werden, indem im Terminal der Aufruf \textit{./simulate-read.sh 12345} durchgeführt wird, ohne dass der Entwickler einen physischen NFC-Reader
...
\paragraph*{Entwicklung der Authentication-Page}
\begin{wrapfigure}{r}{4cm}
  \includegraphics[width=3.8cm]{WelcomeScreenV1.png}
  \caption{Orderstruktur}
  \label{fig:Welcome Screen (Version 1)}
\end{wrapfigure}
Um das in diesem Milsteone erstellte Feature des Abspielens von Musik sinnvoll nutzen zu können musste sich auch mit dir Authentifizierung mit Spotify beschäftigt
werden. Dafür wurde das offene Protokoll oAuth verwendet, welches bereits in \textit{Feathers} abstrahiert ist. Zur Nutzung dieses Protokoll wurde eine
sogenannte \textit{Stragegie} in Form einer Klasse verwendet (in diesem Fall die Klasse SpotifyStrategie), die von der von Feathers bereitgestellten Klasse
OAuthStragegie erbt. \colorbox{red}{JWT}
Zum Anstoß dieses Prozesses wurde auf Basis des Mockups ein View in der Benuterzoberfläche erstellt. Dieser enthält als Steuerelemente nur Infos über den Namen
des Produkts und einen Kurzen Untertitel, sowie den Button, der den OAuth-Prozess startet. Diese Seite wurde mittels des \textit{Vue Routers} so konfiguriert,
dass sie nur aufgerufen wird, wenn der Benutzer nicht angemeldet, also kein JWT-Token vorhanden ist.
$~$ \\
$~$ \\
$~$ \\
%Probleme
...
sollten alle angelegten Tags gruppiert angezeigt werden. Hierfür wurde zuerste eine \textit{component} (\textit{TagEntry.vue}) erstellt, welche Bild und Name des NFC-Tags anzeigt
und in der Anwenung vielseitig widerverwendet werden konnte. Zusätzlich wurde eine \textit{component} mit dem Namen \textit{GroupDropDown.vue} erstellt, welches eine
Gruppe darstellen soll. Pro NFC-Tag in dieser Gruppe wird eine Instanz der \textit{TagEntry.vue component} in das \textit{GroupDropDown.vue} geladen. Zusätzlich erhielt
\textit{GroupDropDown.vue} die Funktionalität es auf- und zuklappen zu können.
