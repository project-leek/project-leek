...
Aufgrund von nicht vorhandener Typisierung wurde sich dann auf die von Microsoft entwickelte Sprache TypeScript geeinigt. Da diese aufgrund des
Aufbaus auf den ECMAScript-6-Standards eine große syntaktische Ähnlichkeit zu JavaScript besitzt, fiel das Erlernen den Entwicklern relativ leicht.
...
Außerdem lässt sich Typescript durch Nutzung der Laufzeitumgebung Node.js auch als Backendsprache verwenden und ersparte dem Team so die Nutzung von verschiedenen Programmiersprachen.

\subsubsection{Frameworks}
Neben der Programmiersprache Typescript setzte das Team verschiedene Frameworks ein, um bei der Entwicklung der Verwaltungsoberfläche mit weniger Aufwand zu entwickeln.

\subsubsection*{Vue.js}
Zur Gestaltung der Verwaltungsoberfläche für die Music-Box, die zum Verwalten der NFC-Tags und steuern der Ausgabegeräte genutzt wird einigte sich das Team auf die Nutzung des
clientseitigen JavaScript-Webframeworks \textit{Vue.js}\footnote{\raggedright\url{https://vuejs.org/}}
Dieses wurde gewählt, da es sehr gut dokumentiert ist und eine flache Lernkurve besitzt, was es gerade für die Gruppenmitglieder mit weniger Erfahrung im Web-Bereich leichter zu erlernen machte.
...
während andere am Logik-Code arbeiteten. Im Vergleich zu anderen Webframeworks gilt es, laut Benchmarks außerdem als sehr performant. \cite{Vue_Performance}

\subsubsection*{Tailwind CSS}
Zur Vereinfachung der Gestaltung der Verwaltungsoberfläche entschied sich das Team für das Utility-First-Frameworks \textit{TailwindCSS}.
Dieses bietet im Vergleich zu anderen CSS-Frameworks wie \textit{Bootstrap} mehr Flexibilität, weil statt vorgefertigten Komponenten Utility-Klassen zur Verfügung gestellt werden,
über die das Design selbst definiert werden kann. Ein weiterer Vorteil ist außerdem, dass das Aussehen innerhalb des HTML-Codes definiert wird und so nicht externe CSS Dateien angelegt werden müssen.
Damit steigt die Übersichtlichkeit, da zur Änderung des Aussehens nicht in externen Dateien gesucht werden muss.
\cite{Tailwind_Vorteile}

\subsubsection*{feathers.js}
Framework serviceorientiert, Echtzeitzugriff Websockets, einfaches CRUD, OAuth

\subsubsection{Reverse-Tunnel (ngrok)}
Um verschlüsselt vom Frontend (öffentlich unter \url{https://project-leek.github.io} erreichbar) auf das jeweilige Backend einer \textit{leek-box} zuzugreifen, wird der Reverse-Tunnel Dienst \textit{ngrok}\footnote{\url{https://ngrok.com/}} verwendet.
...
Es enthält einen Verweis auf die jeweiligen Images, auf denen die Container aufbauen (z. B. leek-backend). Diese Images enthalten bereits alle notwendigen Abhängigkeiten und Programme, sodass eine weitere Installation seitens des Benutzers nicht erforderlich ist.
