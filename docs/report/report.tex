...
Um mit NFC Tags arbeiten zu können, müssen diese gelesen und diese Daten an das Backend übertragen werden.
...
\textit{Evdev}, ein Kernel Modul von Linux, könnte dann zum Abgreifen dieser Keyboard-Eingaben genutzt werden.
Mit \textit{node-evdev}\footnote{https://github.com/sdumetz/node-evdev} kann \textit{evdev} auch mit Node genutzt werden.
Mit dem Fork\footnote{https://github.com/anbraten/node-evdev} von Anbraten wird zusätzlich der Raspberry Pi und die Typescript Unterstützung zur Verfügung gestellt.\\
Ein NFC-Tag hat generell nur eine feste ID.
...
Dafür wäre ein spezieller NFC-Reader, der auch schreiben kann, notwendig.

\subsection{Raspberry Pi}
\subsubsection*{Docker Integration}
Docker Integration hat, wie schon in Abschnitt \ref{abschnitt:docker} beschrieben, sehr viele Vorteile.
...
Um direkt zu Authentifizieren, ob der Nutzer ein Account bei einem jeweiligen Musikservice hat, und welches Abomodell er bei diesem besitzt, bietet es sich an, diesen direkt mit dem jeweiligen Service direkt einloggen zu lassen.
...
Der einfachere Weg hierbei ist es, den Nutzer ein externes Bild verlinken.
Das ist ohne größerer Aufwand möglich.
Sollte der Nutzer eigene Bilder hochladen wollen, wird das ganze etwas komplizierter, da die Daten zum Backend gesendet werden müssten.
