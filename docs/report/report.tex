...
\colorbox{yellow}{Wo Projektstruktur --> Monorepo?}
...
Aufgrund von nicht vorhandener Typisierung wurde dann allerdings die von Microsoft entwickelte Sprache TypeScript für die Projektierung genutzt. Da diese aufgrund des
...
Zur Gestaltung der Verwaltungsoberfläche für die Music-Box, die zum Verwalten der NFC-Tags und steuern der Ausgabegeräte genutzt werden sollte einigte sich das Team außerdem auf die Nutzung des
...
während andere am Logik-Code arbeiteten. Im Vergleich zu anderen Webframeworks gilt es, laut Benchmarks Test Test Test Test Test außerdem sehr performant. \cite{Vue_Performance}

\subsubsection*{Tailwind CSS}
Zur Vereinfachung der Gestaltung der Verwaltungsoberfläche einigte sich das Team auf die Nutzung des Utility-First-Frameworks \textit{TailwindCSS}.
Dieses bietet im Vergleich zu anderen CSS-Frameworks wie \textit{Bootstrap} mehr flexibilität, weil statt vorgefertigten Komponenten Utility-Klassen zur Verfügung gestellt werden, über die die das Design selbst definiert werden kann.
Ein weiterer Vorteil ist außerdem, dass das Aussehen innerhalb des HTML-Codes stattfindet und so nicht extra CSS Dateien angelegt werden müssen. Damit steigt die Übersichtlichkeit, da das Aussehen direkt an dem jeweiligen HTML-Tag vorgenommen wird.
\cite{Tailwind_Vorteile}

\subsubsection{Containervirtualisierung (Docker)}
Um dem Nutzer die Installation der Box möglichst einfach zu machen wird auf die freie Containervirtualisierungssoftware \textit{Docker}\footnote{https://www.docker.com/} gesetzt.
Ohne diese Möglichkeit wäre die Installation aufgrund der 3 Microservices (Backend, Reverse-Proxy und NFC-Reader) sehr aufwändig.
Der Vorteil von Docker besteht darin, dass das Team lediglich ein sogenanntes \textit{docker-compose} file benötigt, in dem die Konfiguration der Docker-Container beschrieben ist.
Es enthält das jeweilige Image, auf dem der Container aufbaut (z. B. ngroc - für den Reverse-Proxy). Dieses Image enthält schon alle notwendigen Abhängigkeiten und Programme, sodass der Benutzer diese nicht
selbst installieren muss. Neben dem Image sind noch Informationen über die weitergeleiteten Ports (zur Kommunikation zwischen den einzelnen Containern und nach \colorbox{red}{draußen}), und die verwendeten
Laufwerke. Neben der einfachen Auslieferung bietet Docker außerdem den Vorteil, dass Fehler, die bei einem Entwickler auftreten bei anderen reproduzierbar sind, da die Software nicht auf unterschiedlichen
Systemen, sondern immer auf dem gleichen System läuft (dem/der Container)
\colorbox{red}{Da nochmal rübergucken}
...
Die IDE ist sehr flexibel und kann mit allen für das Projekt benötigten Dateitypen umgehen. Außerdem ist sie durch eine vielzahl an kostenlos angebotenen Erweiterungen (Extentions) sehr anpassbar an unsere Bedürfnisse.
So wurden in diesem Fall  die Erweiterungen \textit{Vetur} (für Vue.js), \textit{LaTeX Workshop}  (für die Erstellung dieses Berichts), \textit{Tailwind CSS IntelliSense} (Für Auto-Vervollständigung der CSS Klassen von Tailwind) verwendet.
Außerdem bietet \textit{Visual Studio Code} eine komfortable anbindung and das \textit{Windows Subsystem für Linux (WSL)}\footnote{https://code.visualstudio.com/docs/remote/wsl}, welches wir einsetzten, um eine möglichst homogene Arbeitsumgebung innerhalb des Teams zu haben
und nicht nur auf der gleichen IDE sondern auch auf dem einem UNIX-Betriebssystem zu arbeiten. \colorbox{red}{WSL noch erklären?} Dies machte insbesondere die Arbeit mit \textit{Docker} deutlich einfacher.
...
\subsubsection{extern genutzte Libraries}
- Spotify Web API (zum einfachen Zugriff auf Spotify API)
- nedb: Json DB (keine RL nötig, einfach, kostenlos, inkrementell, Feathers Support)
