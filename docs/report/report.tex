...
So gab beispielsweise ein Designer des Unternehmens Feedback zum Entwurf der Benutzeroberfläche
...
Mithilfe der von macio gestellten Projektskizze (siehe \ref{pdf:macioprojektskizze}) haben wir uns folgende Produktvision vorgestellt:
Das Produkt ist eine Musik-Box, die NFC-Chips lesen und Spotify Connect unterstützen soll.
...
Aus der Projektskizze und der Produktvision lassen sich konkreter folgende Mindestanforderungen und entsprechende Klassifikationen an das Produkt ableiten:
\begin{center}
  \begin{tabular}[h]{c|c}
...
Um diese Anforderungen weiter zu definieren hat das Team hierfür User Stories erstellt.
Durch die User Stories wurde sichergestellt, dass beide Parteien (der Kunde und die Entwickler) die Anforderungen auf gleiche Weise interpretieren und verstehen.
Dabei entstand auch ein Priorisierung der Anforderungen bemessen an dem Return of Investment und dem Nutzen für den Endnutzer.
Im weiteren Gespräch mit den Kunden ergeben sich weitere ergänzende Anforderungen an das Produkt:
\begin{center}
  \begin{tabular}[h]{c|c}
    Anforderung & Klassifikation \\
    \hline
...
Durch die agile Entwicklung des Projektes sind diese Anforderungen iterativ von Sprint zu Sprint gewachsen.
Neue implizite Anforderungen sind aus den bestehenden Anforderungen entstanden.
Beispielsweise um den Kostenpunkt von maximal 30€ zu realisieren entstand die Idee einer kleinen kostengünstigeren Version, welche unter 30€ liegt und nur die Grundfunktionen umsetzt, und einer größerer umfangreicheren, die weitere Funktionen beinhaltet wie das Abspielen der Musik über einen eigenen Lautsprecher.
Das Team stieß auch während der Entwicklung auf wiedersprechende Anforderungen.
So wurde sich zum Beispiel gegen bezahlte Services \colorbox{red}{für subdomains (bessere formulierung?)} entschieden auf Kosten der Usability, da dies mehr der Opensource Natur des Projektes entspräche.
Weiter wurden auch die Anforderungen über die Entwicklung hinweg durch das Kundenfeedback am Ende der Sprints immer spezifischer.
