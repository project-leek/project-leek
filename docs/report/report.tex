...
Smarte Spielzeuge gibt es im kommerziellen Bereich viele.
Mit diesem Projekt soll es gegen den Kommerz eine Open-Source-Alternative geschaffen werden.
\subsubsection{Produktvision}
Mithilfe der von macio gestellten Projektskizze (siehe \ref{pdf:macioprojektskizze}) haben erstellte das Team die folgende Produktvision:
Angestrebt wird die Konzeption und Entwicklung einer Musik-Box, die NFC Tags lesen kann und Schnittstellen zu Musik-Streaming-Anbiertern (wie Spotify) besitzt.
...
durch die sichergestellt wurde,  dass beide Parteien (der Kunde und die Entwickler) die Anforderungen  gleich interpretieren und verstanden.
...
Durch die agile Entwicklung des Projektes sind diese Anforderungen iterativ von Sprint zu Sprint gewachsen und neue implizite Anforderungen sind aus den bestehenden Anforderungen entstanden.
Um den Kostenpunkt von maximal 30€ zu realisieren entstand beispielsweise die Idee, einer kleinen kostengünstigeren Version, bei der die Kosten weniger als 30€ betragen und nur die Grundfunktionen umsetzt, und einer größerer umfangreicheren, die weitere Funktionen beinhaltet wie das Abspielen der Musik über einen eigenen Lautsprecher.
Das Team stieß auch während der Entwicklung auf wiedersprechende Anforderungen.
So wurde sich zum Beispiel gegen bezahlte Services \colorbox{red}{für subdomains (bessere formulierung?)} entschieden, was zwar die Usability etwas einschränkte, jedoch mehr dem Open-Source-Gedanken des Projektes entspricht.
