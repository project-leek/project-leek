...
\\
Am Anfag des Zyklus steht die Auswahl eines \textit{Issue} aus der Spalte \textit{ToDo} des \textit{Issue Boards}.
Nach Identifikation weist sich der Entwickler selbst dem Ticket zu. Daraufhin erstellt der verwendete Bot automatisch einen Branch
in dem Schema \textit{[Ticket_Nummer]-[Name_des_Tickets]}. Nun kann mit der Arbeit in \textit{Visual Studio Code} begonnen werden.
\\
Die Änderungen sollten möglichst kleinschrittig aber sinnvoll \textit{comittet} und anschließend gepusht werden.
GitHub Erkennt automatisch die neuen \textit{Commits} und fragt den Entwickler, ob ein neuer \textit{Pull-Request} angelegt werden soll.
Der angelegte \textit{Pull-Request} hat initial den Status \textit{Open}.
Wenn die Aufgaben des \textit{Issues} noch nicht abgeschlossen sind, setzt der Entwickler den Status auf \textit{Draft}.
\\
Sind alle Änderungen abgechlossen wird das Ergebnis der automatisierten Tests (\textit{GitHub Actions}) geprüft. Treten hierbei Fehler auf,
muss der Entwickler diese prüfen und beheben. Sind alle Tests erfolgreich,betätigt der Entwickler die Schaltfläche \textit{Ready for Review}.
Wenn ein anderer Entwickler, diesen \textit{Pull-Request} sieht, führt er ein \textit{Code-Review} durch.
Hierzu läd er sich den Branch in seine IDE und verifiziert die Funktionalität. Anschließend wird die Code-Qualität geprüft.
Hierzu sieht er sich den geänderten Dateien im Pull-Request an. Wenn er Anmerkungen hat, können diese in Form
von Kommentaren an einzelne oder mehrere Zeilen angefügt werden. Sollte der Entwickler Änderungsvorschläge haben, können diese über eine \textit{suggestion}
getätigt werden. Ist der komplette Code geprüft muss der Entwickler entscheiden,
ob er die Änderungen annimmt (\textit{approve}) oder Änderungen anfordern möchte (\textit{Request changes})
\\
Im Falle von Änderungsanforderungen werden diese umgesetzt und ein erneutes Review angefordert. Dies wiederholt sich so oft, bis die Änderungen angenommen werden.
Sind die Änderungen durch zwei \textit{Code-Reivews} bestätigt worden, wird die Änderung mit der Schaltfläche \textit{Mege into master} übertragen.
