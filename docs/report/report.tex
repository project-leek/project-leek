...
Um dieses Modul auch für den Raspberry Pi und mit Typescript nutzbar zu machen, müssten noch Änderungen an diesem Modul durchgeführt werden. 
\\
...
Zur Bereitstellung der Benutzeroberfläche für eine Box soll der Raspberry Pi verwendet werden. 
...
Da die öffentliche IP-Adresse eines privaten Internet-Anschlusses häufig nicht konstant ist, könnte zum Zugiff ein DynDNS Dienst verwende werden, welcher eine feste Domain in die wechselnde IP Adresse des Routers übersetzt.
Zur Umsetzung dieses Punktes mittels DynDNS könnten die in diesem Feld unerfahreneren Entwickler etwas Einarbeitungszeit benötigen, was die Abschätzung des Arbeitsaufwandes sehr abhängig von den Erfahrungen macht.
Alternativ könnte für das Projekt auch \textit{Github.io Pages} genutzt werden, was impliziert, dass jede \textit{Leek-Box} kein eigenes Frontent hostet, sondern dieses auf einem von GitHub bereitgestellten Web-Server öffentlich erreichbar wäre. In diesem Fall müsste die Kommunikation zwischen dem öffentlichen Web-Server und dem Backend auf dem Raspberry Pi implementiert werden.
...
Sollte ein Nutzer sich bei mehreren Musikstreaminganbieter verwenden wollen, muss eine eindeutige Identifikation anhand eines Merkmals sichergestellt werden.
