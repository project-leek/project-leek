...
Die E-Mail wäre hierbei ein geeignetes Merkmal, das einzigartig sein sollte. Über Scopes in der OAuth Anfrage wird dieses in der Regel vom jeweiligen Provider mitgeliefert.
Um die E-Mail als Merkmal zu verwenden, wird vorausgesetzt, dass Nutzer immer die gleiche E-Mail bei den unterschiedlichen Providern nutzen. Dies kann aber nicht immer gewährleistet werden und hängt stark von dem einzelnen Nutzer ab.
Man könnte dem (bereits eingeloggten) Nutzer die Möglichkeit geben, weitere Accounts zu dem bestehenden hinzuzufügen, sodass ein Login über unterschiedliche Provider den gleichen Nutzer einloggen.
...
Sollte bei der Verwendung von Spotify kein Problem sein, da zu jedem Titel / Lied eine Liste von Bildern enthalten sein sollte.\footnote{https://stackoverflow.com/questions/10123804/retrieve-cover-artwork-using-spotify-api}
...
Mit einem Axios Post an das Backend könnte das Bild gesendet werden. \footnote{https://www.digitalocean.com/community/tutorials/vuejs-uploading-vue-picture-input}
...
Über einen API Call an die Spotify API mit dem Endpunkt \textit{/v1""/me""/player""/device} erhält man eine Liste von allen verbundenen Geräten, welche man dann dem Endpunkt zum Abspielen eines Liedes übergeben kann.
...
Die Liste von verbundenen Geräten, die über einen Call an die Spotify API erhalten wird, enthält auch einen eindeutiges Feld \textit{id}, welches sich zusammen mit einem Namen speichern lässt.

\subsubsection{In Cloud Anwendung die jeweilige Box auswählen}
Bei der Ersteinrichtung der Box könnte sich die Cloud Anwendung mit der MAC Adresse oder einem vom Nutzer gesetzten Namen anmelden.
Diese Box wäre dann der Cloud Anwendung bekannt und könnte auch in der Cloud Anwendung ausgewählt werden.
...
Youtube bietet die Möglichkeit, nach Videos zu suchen \footnote{https://developers.google.com/youtube/v3/}. Um diese Videos auf dem Raspberry als Musik abzuspielen würde sich `youtube-dl` zum downloaden der Videos als `.mp3` Dateien und  `omxplayer` zum Abspielen anbieten. Hierfür wäre allerdings ein entsprechender Lautsprecher am Raspberry erforderlich. Ein vergleichbares System zu Spotify Connect existiert derzeit in dieser Form noch nicht für Youtube.
